...
void
tf(char *s) { if (s != NULL && strcmp(s, "") != 0) { free(s); }}
...
pheader(char *s) {
    ...
    fprintf(*fout_ref ? *fout_ref : stdout,
            "/* coverted program name:  %s */ \n\n", s);
    tf(s);
}

/* count the number of occurences of 'c' in 'in_str' */
uint32_t
...
ident_to_pointer(char *s) {
    ...
    while (cur_tok != NULL) {
        ...
        cur_tok = strtok(NULL, ",");
        if (cur_tok != NULL) { strcat(new_buf, ","); }
    }
    return new_buf;
}
...
ident_with_bracket //ident_list_with_bracket
...
%
type <crepr> lit_vals
string_vals scalar_vals
bool_vals

%
type <crepr> one_side_exp
two_side_exp expression
...
%
type <crepr> result_stmt
func_while_stmt func_for_stmt
func_if_stmt
        func_label_stmt
func_ret_stmt ret_val

/* function related */
%
type <crepr> func_decl
func_body func_stmts
func_statement_list func_stmt
...
arglist func_arguments
func_arglist type_only_arguments
...
%
type <crepr> basic_exp
exp_join func_exp_join



/* this is to free each discarted value in case of error. */
%destructor {
/* action */
tf($$);
if(yyerror_count == 0) {
...
IDENT
        KW_LPAR
type_only_arguments KW_RPAR
KW_COLON cdata_with_type
KW_SEMICOLON
        decls
func_body KW_SEMICOLON
        {
...
func_body:
        KW_BEGIN
func_stmts KW_END{$$ = template("{%s}", $2); tf($2);}//{$$ = $2;}
;

/* functions have their own statement decl. as
   they need to accomondate the 'result' statement */
func_stmts: { $$ = ""; }
...
func_statement_list:
        func_stmt
|
func_statement_list KW_SEMICOLON
func_stmt
{
$$ =
template("%s%s", $1, $3);
...
type_decl_single:
        IDENT
KW_EQ cdata_with_type
KW_SEMICOLON
{
$$ =
template("\ttypedef %s %s;", $3, $1);
tf($1);
tf($3);
}
...
cdata_with_type KW_SEMICOLON
        ...
        if (s == NULL)
        {
            tf(s);
            tf($5);
            yyerror("No memory to allocate in order to pointerize");
        }
        else
        {
            $$ = template("\t%s %s;\n", $5, s);
...
tf($4);
tf($6);
yyerror("No memory to allocate in order to bracketize");}
else
{
$$ =
template("\t%s %s;\n", $6, s);
...
/* this is to allow ident[index] sceme */
ident_with_bracket:
        IDENT
                {$$ = $1;}
|
...
/* handle assigment operator (:=) */
assign_stmt:
        ident_with_bracket
KW_OP_ASSIGN exp_join
        {$$ = template("%s = %s;\n", $1, $3); tf($1); tf($3);};
...
for_stmt:
        KW_FOR
ident_with_bracket KW_OP_ASSIGN
exp_join KW_TO
exp_join KW_DO
statement
{
...
|
KW_FOR ident_with_bracket
KW_OP_ASSIGN func_exp_join
KW_DOWNTO func_exp_join
KW_DO func_stmt
        {
                $$ = template("for(%s= %s; %s <= %s; %s--) {\n %s\n}",
        $2, $4, $2, $6, $2, $8);
...
func_proc_call: IDENT
KW_LPAR func_arguments
KW_RPAR
{
$$ =
template("%s(%s)", $1, $3);
...
if(s == NULL) {
tf($3);
yyerror("No memory to pad cdata to ident");
} else
{
$$ = s;
tf($1);
...
|
type_only_arguments KW_SEMICOLON
ident_list KW_COLON
cdata_with_type
{
